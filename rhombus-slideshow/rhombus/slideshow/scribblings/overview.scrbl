...
Here's a ``hello, world'' slide presentation to try running:
...
On the first call to @rhombus(slide), a window is created to show the
slide presentation. As additional slides are registered, they become
available within the presentation. Typically, slides are registered fast
enough that the entire presentation appears ready immediately.

Use Esc or Meta-q/Alt-q/Option-q to end the slide presentation. See
@secref("keybinding") for more information on controlling the
presentation.

@section{Multiple versus Single @rhombus(slide) Calls}

The main @rhombus(slide) function accepts both static and animated picts:
