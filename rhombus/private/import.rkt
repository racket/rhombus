...
(begin-for-syntax
  ...
  (define (extract-prefix mp r)
    ...
    (cond
      [(null? prefixes)
       (syntax-parse mp
         #:datum-literals (lib import-root import-dotted import-spaces file reimport)
...
(define-syntax (rhombus-import-one stx)
  (syntax-parse stx
    #:datum-literals (import-dotted import-root import-spaces)
    ...
    [(_ wrt (import-spaces (space (~and ir (import-root id map))) ...) r (k-form write-placeholder . k-args))
     #`(begin
         (imports-from-root (ir ...) r)
         (k-form wrt . k-args))]
    [(_ wrt (import-spaces (space mp) ... (space-n mp-n)) r k)
     #`(begin
         (rhombus-import-one wrt mp (only-space-in space r) (no-more wrt-placeholder))
         ...
         (rhombus-import-one wrt mp-n (only-space-in space-n r) k))]
    [(_ wrt mp r (k-form write-placeholder . k-args))
     #:with new-mp (collapse-path #'mp #'wrt)
     ...
     #`(begin
         #,(lower-require-clause #'r #'new-mp (and (syntax-e prefix)
                                                   prefix))
    ...
    (for/list ([space-sym (in-list (cons #f (syntax-local-module-interned-scope-symbols)))]
               #:do[(define intro (if space-sym
                    ...
                    (define i (and (or (not as-field?)
                                   ...
                                   (syntax-local-value* space-id import-root-ref)))]
               #:when i)
      (list space-sym
            (syntax-parse i
...
(define-syntax (imports-from-root stx)
  (syntax-parse stx
    [(_ (ir ...) r-parsed)
     (let loop ([irs (syntax->list #'(ir ...))] [covered-ht #hasheq()])
       (syntax-parse (car irs)
         ...
         [(import-root id (map orig-id [key val] ...))
          ...
          #`(begin
              ...
              #,@(if (null? (cdr irs))
                     null
                     (list (loop (cdr irs) new-covered-ht))))]))]))
