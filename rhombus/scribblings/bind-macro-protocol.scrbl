...
@itemlist(
 ...
 functions (in the next bullets).}

 @item{A compile-time ``committer'' function that generates intermediate
 definitions. These definitions happen only after the match is
 ...
 first bullet above). These definitions happen only after the match is
...
three. Operationally, parsing a binding form gets the first function,
and then that function reports the other three along with the static
...
a match-generator, commit-generator, and binding-generator function,
...
@itemlist(
 ...
 @item{A string that is used for reporting a failed match. The string is
  ...
  the binding. For example, when @rhombus(ConsList.cons(x, y), ~bind) is used as a binding
  pattern, a suitable annotation string might be
  @rhombus("matching(ConsList.cons(_, _))") to phrase the binding constraint as an
...
The functions bound with @rhombus(bind.matcher),
