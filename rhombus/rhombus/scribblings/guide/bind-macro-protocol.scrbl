...
@itemlist(
 ...
 @rhombus(::, ~bind) operator. If a binding is repeated via
 @rhombus(..., ~bind), for example, matching will be applied multiple
 times, but one-time definitions are evaluated just once for use in each
 repeated match attempt. Similarly, an annotation check that is repeated
 via @rhombus(List.of, ~annot) or delayed by
 ...
 delayed annotation is checked. Although one-time definitions are
 evaluated once per repeated match, they typically need to be evaluated
 each time an enclosing form is evaluated. For example, if a binding form
 is for the argument of a function, one-time definitions are can be
 evaluated each time the function is called, because bindings for earlier
 arguments are available to the expression. The matcher step (described
 next) can directly refer to definitions generated by the oncer step.}
