...
Similar to the way that an @tech{annotation} can be an
...
@itemlist(
 ...
 @item{A compile-time @deftech{oncer} function that generates definitions to
 ...
 @item{A compile-time @deftech{matcher} function that generates code that
 ...
 @item{A compile-time @deftech{committer} function that generates intermediate
 ...
 useful if some value needs to be computed once and referenced by
 multiple user-visible bindings (which does not work for definitions
 generated in the binder function (described next), as the bindings are scoped to only
 later definition by @rhombus(let)).

 When a binding form is used in a
 match-only position, such as within a @rhombus(matching, ~annot) annotation on
 the right-hand side of @rhombus(is_a), then the committer function is
 not used.}

 @item{A compile-time @deftech{binder} function that generates definitions for
...
search contexts, the @tech{matcher} function (third bullet above)
...
four functions (once for each of the above bullets),
...
These two pieces are assembled into a parenthesized-tuple @tech{syntax object},
...
objects: a representation of ``downward'' static information from the enclosing context and the
parsed binding's data. The result must be a @tech{syntax object} tuple with
the following parts:
...
@itemlist(
 ...
 @item{``Upward'' static information associated with the overall value for a
   ...
   @rhombus(matching, ~annot) annotation operator, for example, as well as propagated
