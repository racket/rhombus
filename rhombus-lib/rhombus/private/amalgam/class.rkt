    ...
    (with-syntax ([((mutable-field-converter mutable-field-annotation-str) ...)
      ...
      (with-syntax ([(set-name-field!/unchecked ...) (for/list ([set-name-field!-id (in-list (syntax->list #'(set-name-field! ...)))]
        ...
        (append
         (list
          #`(define-values (class:name primitive-make-name name? name-field ... set-name-field!/unchecked ...)
              (let-values ([(class:name name name? name-ref name-set!)
                            (#,(if prefab? #'make-struct-type #'make-class-struct-type)
                             'reflect-name
                             ...
                             #,@(cond
                                  [prefab? '()]
                                  [(or (zero? (vector-length method-vtable))
                                       abstract-name)
                                   (list #'#f)]
                                  [else
                                   (list #`(vector #,@(vector->list method-vtable)))]))])
         ...
         (list
          #`(define (name-ref v)
              (define vtable (class-methods-ref v))
              (or vtable
