...
(struct group (col bar-line add-semi? close?) #:transparent)
...
(define (toggle-armor t e)
  (define start (line-start t (send t get-start-position)))
  (define end-start-line (send t position-paragraph (send t get-end-position)))
            ...
            (define (more-nested)
              ...
              (case use-category
                         ...
                         [else
                          ...
                          (case category
                            [(closer comma-operator semicolon-operator)
                             (close-immediately init-pos)]
                            [else
                             (define init-col (- init-pos (line-start t init-pos)))
                             (cond
                               [(and (pair? (parse-state-gs state))
                ...
                [(opener)
                 ...
                 (loop next-pos e #t
                       (make-parse-state (list (make-group (- next-pos (line-start t next-pos))
                                                           #:add-semi? #f
                                                           #:close? #f))
  ...
  (void
   (for/fold ([end end]) ([ins (in-list inserts)])
     ...
     (define str (case (cdr ins)
                   ...
                   [(semi) ";"]
                   [else "??"]))
     (define col (- pos (line-start t pos)))
