...
(struct group-state (closer         ; expected closer: a string+stx, EOF, or column
                     paren-immed?   ; immediately in `()` or `[]`?
  ...
  (let loop ([status 'initial])
    ...
    (case (token-name tok)
      ...
      [(fail)
       (error "reader error")]
      [(s-exp)
       (cons (finish-s-exp tok in) (loop 'continuing))]
      [else
       (cons tok (loop new-status))])))

(define (finish-s-exp open-tok in)
...
(define (parse-all in)
  (define l (parameterize ([current-lexer-source (object-name in)])
              (lex-all in)))
  (if (null? l)
      eof
      (parse-top-groups l)))
