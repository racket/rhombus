...
@tech{blocks} (a generalization of ``paragraphs'') that have
...
@itemlist(
 ...
 specific sense defined below);}
  ...
  @item{a sequence of @tech{blocks} that determine the part's text, also
 call a @deftech{flow} (i.e., a list of @tech{blocks} is a @tech{flow} in
 general); and}
...
A @deftech{block} is a @deftech{paragraph}, a @deftech{nested flow}
(which itself contains a @tech{flow} sequence of @tech{blocks}), a
...
functions) explicitly create other kinds of blocks. Each block also has
@tech{style}.

A @tech{paragraph} is the only kind of block whose content is not other
blocks, and the technical term @deftech{content} refers to values that
...
element cannot contain @tech{blocks} or @tech{parts}.

A @deftech{style} controls the rendering of an element or block. A
...
is rendered into, but various style names and properties are intended to
...
represented as a structured object.
